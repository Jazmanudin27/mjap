    ...
    <div class="col-md-12">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="fa fa-warehouse"></i> Laporan Persediaan</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('cetakLaporanPersediaan') }}" method="POST" autocomplete="off" target="_blank">
                    @csrf
                    <div class="row g-2 mt-2">
                        ...
                        <div class="col-md-6">
                            <input type="date" name="mulai" class="form-control form-control-sm"
                                value="{{ request('mulai') ?? date('Y-m-01') }}" required>
                        </div>
                        <div class="col-md-6">
                            <input type="date" name="akhir" class="form-control form-control-sm"
                                value="{{ request('akhir') ?? date('Y-m-d') }}" required>
                        </div>
                    </div>
