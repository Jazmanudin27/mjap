        ...
        <div class="col-6">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="fa fa-box"></i> Laporan Rekap Tagihan</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('cetakRekapTagihan') }}" method="POST" target="_blank" autocomplete="off">
                        @csrf
                        <div class="row g-2 mt-3">
                            <div class="col-md-12">
                                <div class="custom-datepicker-wrapper">
                                    <input type="date" class="form-control form-control-sm" id="tanggal"
                                        name="tanggal" placeholder="Tanggal Sampai" value="{{ request('tanggal') }}"
                                        required>
                                </div>
                            </div>
                        </div>
                                ...
                                <select name="wilayah" class="form-select2 form-select-sm" required>
                                    ...
                                    @foreach ($wilayah as $w)
                                        <option value="{{ $w->kode_wilayah }}"
                                            {{ request('wilayah') == $w->kode_wilayah ? 'selected' : '' }}>
                                            {{ $w->nama_wilayah }}
                                        </option>
                                    @endforeach
                                </select>
