...
@section('titlepage', 'Edit Barang Masuk')
@section('contents')
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="card-header-gradient text-center py-2">
                <h4 class="mb-0 fw-semibold">Edit Barang Masuk</h4>
            </div>

            <form id="formBarangMasuk" action="{{ route('updateMutasiBarangMasuk', $transaksi->kode_transaksi) }}"
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-section">
                            ...
                            <label class="form-label small mt-2">Sumber</label>
                            <select name="sumber" class="form-select form-select-sm" required>
                                <option value="">-- Pilih Sumber --</option>
                                <option value="Repack" {{ $transaksi->sumber == 'Repack' ? 'selected' : '' }}>Repack
                                </option>
                                <option value="Retur Pengganti"
                                    {{ $transaksi->sumber == 'Retur Pengganti' ? 'selected' : '' }}>Retur Pengganti
                                </option>
                                <option value="Penyesuaian" {{ $transaksi->sumber == 'Penyesuaian' ? 'selected' : '' }}>
                                    Penyesuaian</option>
                                <option value="Lainnya" {{ $transaksi->sumber == 'Lainnya' ? 'selected' : '' }}>Lainnya
                                </option>
                            </select>

                            <label class="form-label small mt-2">Keterangan</label>
                            <textarea name="keterangan" rows="3" class="form-control form-control-sm">{{ $transaksi->keterangan }}</textarea>
                        </div>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Load data awal (mode edit)
            if (dataAwalBarang.length > 0) {
                keranjangBarangMasuk = dataAwalBarang.map(item => ({
            ...
            // Saat barang dipilih, ambil satuan
            ...
            // Saat satuan dipilih, simpan ID ke hidden input
            ...
            // Jika qty diedit manual
            $(document).on('input', '.qtyMasuk', function() {
                hitungTotalBarangMasuk();
            });

            // Hitung total & simpan keranjang ke hidden input
            ...
            $('form').on('submit', function(e) {
                ...
                if (keranjangBarangMasuk.some(item => !item.satuan_id)) {
                    e.preventDefault();
